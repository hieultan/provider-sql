...
const (
	errNotSupported = "%s not supported by doris client"
)
...
// New returns a new Doris database client.
...
// ExecTx is unsupported in Doris.
func (c dorisDB) ExecTx(ctx context.Context, ql []xsql.Query) error {
	return errors.Errorf(errNotSupported, "transactions")
}

// Exec the supplied query.
func (c dorisDB) Exec(ctx context.Context, q xsql.Query) error {
...
func (c dorisDB) Query(ctx context.Context, q xsql.Query) (*sql.Rows, error) {
	d, err := sql.Open("mysql", c.dsn)
	if err != nil {
		return nil, err
	}
...
func (c dorisDB) Scan(ctx context.Context, q xsql.Query, dest ...interface{}) error {
...
func (c dorisDB) GetConnectionDetails(username, password string) managed.ConnectionDetails {
...
// QuoteIdentifier for Doris queries
func QuoteIdentifier(id string) string {
	return "`" + strings.ReplaceAll(id, "`", "``") + "`"
}

// QuoteValue for Doris queries
func QuoteValue(id string) string {
	return "'" + strings.ReplaceAll(id, "'", "''") + "'"
}

// SplitUserHost splits a Doris user by name and host

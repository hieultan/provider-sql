...
import (
	...
	"github.com/crossplane-contrib/provider-sql/apis/doris/v1alpha1"
	mysqlv1alpha1 "github.com/crossplane-contrib/provider-sql/apis/mysql/v1alpha1"
	"github.com/crossplane-contrib/provider-sql/pkg/clients/doris"
...
func Setup(mgr ctrl.Manager, o xpcontroller.Options) error {
	...
	reconcilerOptions := []managed.ReconcilerOption{
		managed.WithExternalConnecter(&connector{kube: mgr.GetClient(), usage: t, newDB: doris.New}),
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	var mcfg *mysqlv1alpha1.TLSConfig
	if pc.Spec.TLSConfig != nil {
		m := mysqlv1alpha1.TLSConfig{
			CACert:             mysqlv1alpha1.TLSSecret{SecretRef: pc.Spec.TLSConfig.CACert.SecretRef},
			ClientCert:         mysqlv1alpha1.TLSSecret{SecretRef: pc.Spec.TLSConfig.ClientCert.SecretRef},
			ClientKey:          mysqlv1alpha1.TLSSecret{SecretRef: pc.Spec.TLSConfig.ClientKey.SecretRef},
			InsecureSkipVerify: pc.Spec.TLSConfig.InsecureSkipVerify,
		}
		mcfg = &m
	}
	tlsName, err := tls.LoadConfig(ctx, c.kube, providerConfigName, pc.Spec.TLS, mcfg)
	if err != nil {
		return nil, errors.Wrap(err, errTLSConfig)
	}

	return &external{
		db:   c.newDB(s.Data, tlsName),
		kube: c.kube,
	}, nil
}
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	username, host := doris.SplitUserHost(meta.GetExternalName(cr))
...
func (c *external) executeCreateUserQuery(ctx context.Context, username string, host string, resourceOptionsClauses []string, pw string) error {
	...
	query := fmt.Sprintf(
		...
		doris.QuoteValue(username),
		doris.QuoteValue(host),
		doris.QuoteValue(pw),
		resourceOptions,
	)

	if err := doris.ExecWrapper(ctx, c.db, doris.ExecQuery{Query: query, ErrorValue: errCreateUser}); err != nil {
		return err
	}
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	if len(rochanged) > 0 {
		...
		if err := doris.ExecWrapper(ctx, c.db, doris.ExecQuery{Query: query, ErrorValue: errUpdateUser}); err != nil {
...
func (c *external) UpdatePassword(ctx context.Context, cr *v1alpha1.User, username, host string) (managed.ConnectionDetails, error) {
	...
	if pwchanged {
		query := fmt.Sprintf("ALTER USER %s@%s IDENTIFIED BY %s", doris.QuoteValue(username), doris.QuoteValue(host), doris.QuoteValue(pw))
		if err := doris.ExecWrapper(ctx, c.db, doris.ExecQuery{Query: query, ErrorValue: errUpdateUser}); err != nil {
			return managed.ConnectionDetails{}, err
		}
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) error {
	...
	query := fmt.Sprintf("DROP USER IF EXISTS %s@%s", doris.QuoteValue(username), doris.QuoteValue(host))
	if err := doris.ExecWrapper(ctx, c.db, doris.ExecQuery{Query: query, ErrorValue: errDropUser}); err != nil {
		return err
	}

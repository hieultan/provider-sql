...
const (
	// CredentialsSourceDorisConnectionSecret indicates that a provider
	// should acquire credentials from a connection secret written by a managed
	// resource that represents a Doris server.
	CredentialsSourceDorisConnectionSecret xpv1.CredentialsSource = "DorisConnectionSecret"
)

// ProviderCredentials required to authenticate.
type ProviderCredentials struct {
	// Source of the provider credentials.
	// +kubebuilder:validation:Enum=DorisConnectionSecret
	Source xpv1.CredentialsSource `json:"source"`

	// A CredentialsSecretRef is a reference to a Doris connection secret
